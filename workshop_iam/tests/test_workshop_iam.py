...
class TestWorkshopIAM(unittest.TestCase):
    ...
    def test_project_ids_count_and_uniqueness(self):
        ...
        self.assertEqual(len(project_matches), 40, f"Expected 40 project IDs, found {len(project_matches)}")
        self.assertEqual(len(set(project_matches)), 40, "All 40 project IDs must be unique")

        for i in range(1, 41):
            expected_project = f"afsummit2026-workshop-{i}"
            self.assertIn(expected_project, project_matches, f"Missing {expected_project}")
    ...
    def test_least_privilege_roles(self):
        ...
        disallowed_roles = ["roles/owner", "roles/editor", "roles/resourcemanager.organizationAdmin"]
        for disallowed in disallowed_roles:
            self.assertNotIn(disallowed, role_matches, f"Overly permissive role {disallowed} found")
        ...
        for req in required_roles:
            self.assertIn(req, role_matches, f"Missing expected least privilege role {req}")
